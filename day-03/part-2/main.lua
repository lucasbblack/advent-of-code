...
function get_wire_points(wire)
    local wire_points = {{x=0, y=0, steps=0}}
    ...
    for word in string.gmatch(wire, '([^,]+)') do
        ...
        local point = {x = prevPoint.x, y = prevPoint.y, steps = prevPoint.steps}
        if direction == "R" then
...
for i = 1, #segments_a do
	...
	for j = 1, #segments_b do
		...
		if r then
			local p = {steps = 0}
			-- first vertical line test
			if s1.a.x == s1.b.x then
				p.x = s1.a.x
				p.steps = p.steps + math.abs(s1.a.y - s2.a.y)
			end
			-- first horizontal line test
			if s1.a.y == s1.b.y then
				p.y = s1.a.y
				p.steps = p.steps + math.abs(s1.a.x - s2.a.x)
			end

			-- second vertical line test
			if s2.a.x == s2.b.x then
				p.x = s2.a.x
				p.steps = p.steps + math.abs(s2.a.y - s1.a.y)
			end
			-- second horizontal line test
			if s2.a.y == s2.b.y then
				p.y = s2.b.y
				p.steps = p.steps + math.abs(s2.a.x - s1.a.x)
			end

			

			p.steps = p.steps + math.min(s1.a.steps, s1.b.steps)
					 	+ math.min(s2.a.steps, s2.b.steps)

			table.insert(intersects, p.steps)
		end
	end
end
